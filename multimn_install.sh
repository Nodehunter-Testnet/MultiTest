...
echo -e "\n====================================================================================\
         ...
         \n   ${GRAY}██║ ╚═╝ ██║ ╚██████╔╝ ██████╗   ██║    ██████╗ ${NC}██║ ╚═╝ ██║██║ ╚████║  \
         \n   ${GRAY}╚═╝     ╚═╝  ╚═════╝  ╚═════╝   ╚═╝    ╚═════╝ ${NC}╚═╝     ╚═╝╚═╝  ╚═══╝  \
         \n                                ╗ made by ${GREEN}ElysiumRoyale${NC} ╔\
         \n                   Credit: ${YELLOW}Credit for this script to neo3587${NC}\
         \n           Source: ${CYAN}https://github.com/Nodehunter-Testnet/MultiTest${NC}\
         \n====================================================================================\
         \n                                                   "

# Update the URL to point to your repository's raw script
multimn_update=$(curl -s https://raw.githubusercontent.com/Nodehunter-Testnet/MultiTest/main/multi-mn-test.sh)

if [[ -f /usr/bin/multimn && ! $(diff -q <(echo "$multimn_update") /usr/bin/multimn) ]]; then
    echo_json_upd "${GREEN}multimn${NC} is already updated to the latest version" 0
else
    ...
    if [[ $update == "1" ]]; then
        echo_json_upd "${GREEN}multimn${NC} updated to the latest version, pretty fast, right?" 1
    else
